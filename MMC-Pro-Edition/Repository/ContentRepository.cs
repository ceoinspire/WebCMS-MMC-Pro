    ...
    public class ContentRepository
    {
        ...
        public List<ContentTypeVM> ContentType()
        {
            return _con.ContentType.ToList().Select(x => new ContentTypeVM()
            {
                Id = x.Id,
                Name = x.Name,
                ContentTypeSlug = x.TypeSlug
            }).ToList();
        }
        public string ContentNameByContentId(int id)
        {
            return _con.ContentType
                .Where(x => x.Id == id)
                .Select(x => x.Name)
                .FirstOrDefault();
        }
        ...
        public string CreateContent(string cTitle, string cType, int UserId, int WebsiteId, GeneratedContentModel aiRes)
        {
